...
export function normalizeWebSettings(settings) {
  return {
    ...
    cod_charge: Number(settings?.cod_charge ?? settings?.codCharge ?? 0) || 0,
    online_payment_discount_percent: Number(settings?.online_payment_discount_percent ?? settings?.onlinePaymentDiscountPercent ?? 10) || 10,
  };
}
