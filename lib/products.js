...
function normalizeSize(size) {
  ...
  const discountPrice =
    size.discount_price !== undefined
      ? (size.discount_price !== null ? toNumber(size.discount_price) : null)
      : (size.discountPrice !== undefined
          ? (size.discountPrice !== null ? toNumber(size.discountPrice) : null)
          : null);
...
export function normalizeProduct(product) {
  ...
  const firstSizeWithPrice = sizes.find((size) => size.price > 0);
  const firstSizePrice = firstSizeWithPrice?.price;
  const firstSizeDiscountPrice = firstSizeWithPrice?.discount_price;
  ...
  const productDiscountPrice =
    product.discount_price !== undefined
      ? (product.discount_price !== null ? toNumber(product.discount_price) : null)
      : (firstSizeDiscountPrice !== undefined ? firstSizeDiscountPrice : null);
...
export const DEFAULT_PRODUCTS_PER_PAGE = 40;
...
/** Products for a parent category and its subcategories (deduped). */
export async function getProductsByCategoryTree(category) {
  const parentId = category?._id ?? category?.id ?? category?.slug;
  const childIds = Array.isArray(category?.children)
    ? category.children
        .map((child) => child?._id ?? child?.id ?? child?.slug)
        .filter(Boolean)
    : [];
  const categoryIds = [
    ...new Set([parentId, ...childIds].filter(Boolean).map(String)),
  ];

  if (!categoryIds.length) return [];
  if (categoryIds.length === 1) {
    return getProductsByCategory(categoryIds[0]);
  }
